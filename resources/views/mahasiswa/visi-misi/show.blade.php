...
@section('content')
    <div class="container-xl">
        ...
        <h1 class="app-page-title">Kuesioner Visi Misi
        ...
        <div class="row">
            <div class="col-12">
                <div class="app-card d-flex flex-column shadow-sm">
                    ...
                    @if (!$filled)
                        <form class="settings-form" action="{{ route('mahasiswa.visi-misi.store', $kuesioner->id) }}"
                            method="POST">
                            @csrf
                            <div class="app-card-body py-2 px-4">
                                @foreach ($kuesioner->pertanyaan as $key => $pertanyaan)
                                    <div class="row mt-3">
                                        ...
                                        <div class="col-12 col-md-9">
                                            ...
                                            @if ($pertanyaan->tipe == 1)
                                                @foreach ($pertanyaan->jawaban as $jawaban)
                                                    <div class="form-check mb-1">
                                                        <div class="radio">
                                                            <label for="jawaban{{ $jawaban->id }}"
                                                                class="form-check-label " style="">
                                                                <input type="radio" id="jawaban{{ $jawaban->id }}"
                                                                    class="form-check-input"
                                                                    name="responden[{{ $key }}][jawaban_visi_misi_id]"
                                                                    value="{{ $jawaban->id }}"
                                                                    {{ old('responden.' . $key . '.jawaban_visi_misi_id') == $jawaban->id ? 'checked' : '' }}>{{ $jawaban->jawaban }}
                                                            </label>
                                                        </div>
                                                        <input type="hidden"
                                                            name="responden[{{ $key }}][pertanyaan_visi_misi_id]"
                                                            value="{{ $pertanyaan->id }}">
                                                    </div>
                                                    <div>
                                                        <small class="text-danger">
                                                            {{ $errors->first('responden.' . $key . '.jawaban_visi_misi_id') }}
                                                        </small>
                                                    </div>
                                                @endforeach
                                            @elseif($pertanyaan->tipe == 2)
                                                @foreach ($pertanyaan->jawaban as $jawaban)
                                                    <div class="form-check mb-1">
                                                        <input class="form-check-input" type="checkbox"
                                                            ...
                                                            name="responden[{{ $key }}][jawaban_visi_misi_id][]"
                                                            value="{{ $jawaban->id }}"
                                                            {{ old('responden.' . $key . '.jawaban_visi_misi_id') == $jawaban->id ? 'checked' : '' }}>
                                ...
                                <div class="row mt-3">
                                    <div class="col col-md-3">
                                        <label class=" form-control-label" style="font-weight:bold">
                                            Mohon tuliskan saran terkait Visi, Misi, Tujuan Itenas
                                        </label>
                                    </div>
                                    <div class="col-12 col-md-9">
                                        <div class="form-floating">
                                            <textarea class="form-control @error('saran') is-invalid @enderror" name="saran" placeholder="Masukkan saran Anda"
                                                id="saran">{{ old('saran') }}</textarea>
