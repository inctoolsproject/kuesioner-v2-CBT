...
@section('content')
    <div class="container-xl">
        ...
        <h1 class="app-page-title">Kuesioner Fakultas
        ...
        <div class="app-card alert shadow-sm mb-4 border-left-decoration">
            <div class="inner">
                <div class="app-card-body p-3 p-lg-4">
                    <h3 class="mb-3">Instrumen Penilaian Pelayanan Fakultas Terhadap Dosen
                        {{ substr_replace($kuesioner->semester, '/', 4, 0) . ' - ' . $kuesioner->kegiatan }}
                    </h3>
                    <div class="row gx-5 gy-3">
                        <div class="col-12">
                            <div>
                                Kuesioner ini menanyakan pendapat anda mengenai pelayanan fakultas terhadap tenaga pendidik.
                                Pengumpulan data menggunakan kuesioner ini bertujuan mengukur keefektifan pelayanan fakultas
                                yang telah dilakukan. Berikan tanggapan berdasarkan pendapat sendiri dan bukan
        ...
        <div class="row">
            <div class="col-12">
                <div class="app-card d-flex flex-column shadow-sm">
                    ...
                    @if (!$filled)
                        <form class="settings-form" action="{{ route('dosen.fakultas.store', $kuesioner->id) }}"
                            method="POST">
                            @csrf
                                                    ...
                                                    <div class="radio">
                                                        <label for="jawaban{{ $jawaban->id }}" class="form-check-label "
                                                            ...
                                                            <input type="radio" id="jawaban{{ $jawaban->id }}"
                                                                ...
                                                                name="responden[{{ $key }}][jawaban_fakultas_id]"
                                                                value="{{ $jawaban->id }}"
                                                                {{ old('responden.' . $key . '.jawaban_fakultas_id') == $jawaban->id ? 'checked' : '' }}
                                                                required>{{ $jawaban->jawaban }}
                                                        </label>
                                                    </div>
                                                    <input type="hidden"
                                                        name="responden[{{ $key }}][pertanyaan_fakultas_id]"
                                                        value="{{ $pertanyaan->id }}">
                                                </div>
                                                <div>
                                                    <small class="text-danger">
                                                        {{ $errors->first('responden.' . $key . '.jawaban_fakultas_id') }}
                                                    </small>
                                                </div>
                                            @endforeach
                                        </div>
                                    ...
                                    <div class="col-12 col-md-9">
                                        <div class="form-floating">
                                            <textarea class="form-control @error('select-matkul') is-invalid @enderror" name="saran"
                                                placeholder="Masukkan saran Anda" id="saran">{{ old('saran') }}</textarea>
                    ...
                    @else
                        <div class="app-card-body py-2 px-4">
                            <div class="alert alert-success" role="alert">
                                Anda sudah mengisi kuesioner sarana prasarana pada periode
                                <b>{{ substr_replace($kuesioner->semester, '/', 4, 0) . ' - ' . $kuesioner->kegiatan }}</b>
                                !
                            </div>
                            @foreach ($kuesioner->pertanyaan as $key => $pertanyaan)
                                <div class="row mt-3">
                                                ...
                                                <div class="radio">
                                                    <label for="jawaban{{ $jawaban->id }}" class="form-check-label "
                                                        ...
                                                        <input type="radio" id="jawaban{{ $jawaban->id }}"
                                                            ...
                                                            name="responden[{{ $key }}][jawaban_fakultas_id]"
                                                            value="{{ $jawaban->id }}"
                                                            @if ($jawaban->id == $filled->detail[$key]->jawaban_fakultas_id) checked @endif readonly
