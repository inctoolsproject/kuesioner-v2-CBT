...
@section('content')
    <div class="container-xl">
        ...
        <h1 class="app-page-title">Kuesioner LP2M
        ...
        <div class="app-card alert shadow-sm mb-4 border-left-decoration">
            <div class="inner">
                <div class="app-card-body p-3 p-lg-4">
                    <h3 class="mb-3">Instrumen Penilaian Pelayanan LP2M Terhadap Dosen
                        {{ substr_replace($kuesioner->semester, '/', 4, 0) . ' - ' . $kuesioner->kegiatan }}
                    </h3>
                    <div class="row gx-5 gy-3">
                        <div class="col-12">
                            <div>
                                Kuesioner ini menanyakan pendapat anda mengenai pelayanan LP2M terhadap tenaga pendidik.
                                Pengumpulan data menggunakan kuesioner ini bertujuan mengukur keefektifan pelayanan LP2M
        ...
        <div class="row">
            <div class="col-12">
                <div class="app-card d-flex flex-column shadow-sm">
                    ...
                    @if (!$filled)
                        <form class="settings-form" action="{{ route('dosen.lp2m.store', $kuesioner->id) }}" method="POST">
                            @csrf
                                                    ...
                                                    <div class="radio">
                                                        <label for="jawaban{{ $jawaban->id }}" class="form-check-label "
                                                            ...
                                                            <input type="radio" id="jawaban{{ $jawaban->id }}"
                                                                ...
                                                                name="responden[{{ $key }}][jawaban_lp2m_id]"
                                                                value="{{ $jawaban->id }}"
                                                                {{ old('responden.' . $key . '.jawaban_lp2m_id') == $jawaban->id ? 'checked' : '' }}
                                                                required>{{ $jawaban->jawaban }}
                                                        </label>
                                                    </div>
                                                    <input type="hidden"
                                                        name="responden[{{ $key }}][pertanyaan_lp2m_id]"
                                                        value="{{ $pertanyaan->id }}">
                                                </div>
                                                <div>
                                                    <small class="text-danger">
                                                        {{ $errors->first('responden.' . $key . '.jawaban_lp2m_id') }}
                                                    </small>
                                                </div>
                                            @endforeach
                                        </div>
                                                ...
                                                <div class="radio">
                                                    <label for="jawaban{{ $jawaban->id }}" class="form-check-label "
                                                        ...
                                                        <input type="radio" id="jawaban{{ $jawaban->id }}"
                                                            ...
                                                            name="responden[{{ $key }}][jawaban_lp2m_id]"
                                                            value="{{ $jawaban->id }}"
                                                            @if ($jawaban->id == $filled->detail[$key]->jawaban_lp2m_id) checked @endif readonly
